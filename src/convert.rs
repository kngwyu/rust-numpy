...
pub trait IntoPyArray {
    type Item: TypeNum;
    fn into_pyarray(self, Python) -> &PyArray<Self::Item, RsHeap<Self::Item>>;
}

impl<T: TypeNum> IntoPyArray for Box<[T]> {
    type Item = T;
    fn into_pyarray(self, py: Python) -> &PyArray<Self::Item, RsHeap<T>> {
        let dims = [self.len()];
...
impl<A: TypeNum, D: Dimension> IntoPyArray for Array<A, D> {
    type Item = A;
    fn into_pyarray(self, py: Python) -> &PyArray<Self::Item, RsHeap<A>> {
    ...
    ($($N: expr)+) => {
        $(
            impl<T: TypeNum> IntoPyArray for [T; $N] {
                type Item = T;
                fn into_pyarray(self, py: Python) -> &PyArray<T, RsHeap<T>> {
                    let dims = [$N];
